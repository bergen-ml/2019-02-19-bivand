...
[\textcolor{mLightBrown}{OpenEO}](http://openeo.org/about/) proposes proof-of-concept client-server API approaches (and [\textcolor{mLightBrown}{this issue}](https://github.com/Open-EO/openeo-r-client/issues/25))
...
\begincols
\begincol{0.48\textwidth}
Interpolation from data locations to other locations, also for re-sampling data resolution for raster data, is often computationally intensive. 
[\textcolor{mLightBrown}{Predicive Soil Mapping with R}](https://envirometrix.github.io/PredictiveSoilMapping/) uses random forest approaches to prediction, but this isn't new, see [\textcolor{mLightBrown}{this example from 2003}](https://www.r-project.org/conferences/DSC-2003/Proceedings/FurlanelloEtAl.pdf). 
\endcol
...
sic.367SP <- sic.allSP[which(!rownames(sic.all$coords) %in% rownames(sic.100$coords)),]
library(automap)
...
library(ranger)
m.precip <- ranger(fm0, rm.precip, quantreg=TRUE, num.trees=150, seed=1, keep.inbag=TRUE)
res$rf_pred <- predict(m.precip, rm.precip1, type="response")$predictions
res$rf_sd1 <- predict(m.precip, rm.precip1, type="se", se.method="infjack")$se
res$rf_sd2 <- predict(m.precip, rm.precip1, type="se", se.method="jack")$se
```
...
\begincols
\begincol{0.48\textwidth}
[\textcolor{mLightBrown}{Spatial CV}](https://geocompr.robinlovelace.net/spatial-cv.html#intro-cv)
\endcol
